...
function auto_safeconvert(datajson) {
    ...
    for (var i = 0; i < a_to_va; i++) {
        ...
        while (a_to_vh == false && j < a_to_vg) {
            a_to_vh = extractDomain(a_to_ve[i].href);
            a_to_vi = extractDomain(a_to_vg[j]);
            if (a_to_vi.match(a_to_vf[j]) || !a_to_vi || !a_to_vi.match("http")) { a_to_vh = true; }
            j++;
        }
        if (a_to_vh == false) {
            var encryptedUrl = Base64.encode(a_to_vi);
            a_to_ve[i].href = links[randindex] + "#?o=" + encryptedUrl;
    ...
    if (a_to_vj) { a_to_vj.innerHTML += a_to_vb; }
    if (a_to_vk) { a_to_vk.innerHTML += a_to_va; }
}
